...
class JobKnowledgeBase:
    ...
    def add_company_info(self, company_info: Dict[str, Any]):
        ...
        self.vectorstore.add_texts(
            texts=texts,
            metadatas=[{"source": "company_info", "company_id": company_info.get('id')} for _ in texts]
        )
    ...
    def add_interview_questions(self, questions: List[Dict[str, Any]]):
        ...
        for q in questions:
            ...
            self.vectorstore.add_texts(
                texts=texts,
                metadatas=[{"source": "interview_question", "question_id": q.get('id')} for _ in texts]
            )
    ...
    def get_relevant_interview_questions(self, job_title: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        results = self.vectorstore.similarity_search(
            job_title,
            k=k,
            filter={"source": "interview_question"}
        )
        return [doc.metadata for doc in results] 
 ...
 */
export async function parseLinkedInProfileWithLangChain(htmlContent: string): Promise<LinkedInProfile | null> {
  if (!htmlContent) {
    return null;
  }
 ...
 * @returns Structured LinkedIn profile data
 */
export async function fetchLinkedInProfileWithLangChain(linkedInUrl: string): Promise<LinkedInProfile | null> {
  if (!linkedInUrl) {
    return null;
  }

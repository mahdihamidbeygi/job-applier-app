...
export async function generateResumeFromTemplate(data: ResumeData): Promise<string> {
...
`)
  ...
  if (data.education && data.education.length > 0) {
    const educationSection = data.education
      .map((edu: ResumeData['education'][0]) => `
### ${edu.degree}
...
`)
  ...
  if (data.projects && data.projects.length > 0) {
    const projectsSection = data.projects
      .map((project: ResumeData['projects'][0]) => `
### ${project.name}
- ${project.description}
...
`)
  ...
  if (data.certifications && data.certifications.length > 0) {
    const certificationsSection = data.certifications
      .map((cert: ResumeData['certifications'][0]) => `- ${cert.name} | ${cert.issuer} | ${cert.date}`)
      .join('\n');

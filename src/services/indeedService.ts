...
export class IndeedService {
  ...
  private readonly pageTimeout = 60000; // 60 seconds

  /**
   * Search for jobs using Indeed web scraping
   */
  async searchJobs(params: IndeedJobSearchParams): Promise<Job[]> {
    let retryCount = 0;
    let browser;
   ...
   */
  private buildSearchUrl(params: IndeedJobSearchParams): string {
  ...
  /**
   * Parse the posted date string into a Date object
   */
  private parsePostedDate(dateStr: string): Date {
    const now = new Date();
    const matches = dateStr.match(/(\d+)\s*(day|hour|minute|second)s?\s+ago/);

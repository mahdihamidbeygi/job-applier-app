...
export class JobService {
  ...
  async applyToJob(job: Job, resumePath: string, coverLetter?: string): Promise<ApplicationResult> {
    if (!job.isExternal) {
  ...
  private async applyEasyApply(job: Job, resumePath: string, coverLetter?: string): Promise<ApplicationResult> {
  ...
  private async applyExternal(job: Job, resumePath: string, coverLetter?: string): Promise<ApplicationResult> {
    const browser = await puppeteer.launch({ headless: true });
    const page = await browser.newPage();
